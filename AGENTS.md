...
Source Node engines: `>=22.5.0 <25.0.0` because source runs use Node's built-in `node:sqlite`. Published standalone/npm installs bundle a compatible Node runtime. There is a hard exit on Node 25.x (see `src/bin/node-version-check.ts`).
...
- `src/db/` — `DatabaseConnection`, `QueryBuilder` (prepared statements), `schema.sql`. Backed by Node's built-in `node:sqlite` with WAL/FTS5 support; `codegraph status` surfaces this backend.
...
- `instructions-template.ts` is the agent-agnostic instructions file written to each target (e.g. `AGENTS.md`, `.cursor/rules/codegraph.mdc`, `~/.codex/AGENTS.md`, `~/.config/opencode/AGENTS.md`). It explicitly says "trust codegraph results, don't re-verify with grep" — earlier versions prescribed Claude-specific "spawn an Explore agent" and confused other agents.
- `claude-md-template.ts` is the legacy Claude-only re-export shim, retained for compatibility paths.
...
- Connect / run from the Mac host using the user/key from `.parallels`. For multi-line work, pipe PowerShell over stdin and **refresh PATH from the registry** first (sshd's session has a stale PATH after winget installs):
  ```
  ssh <user>@<guest_ip> "powershell -NoProfile -ExecutionPolicy Bypass -Command -" <<'PS'
...
- `docs/dev/policies/0005-git-worktree-hygiene.md`
- `docs/dev/policies/0006-commit-history-discipline.md`
- `docs/dev/policies/0007-branch-and-integration-strategy.md`
- `docs/dev/policies/0008-commit-and-push-cadence.md`
- `docs/dev/policies/0009-versioning-and-release.md`
- `docs/dev/policies/0010-turn-closeout.md`
- `docs/dev/policies/0011-subagent-workflow-optimization.md`
- `docs/dev/policies/0012-validation-and-handoff.md`
- `docs/dev/policies/0013-parallel-plan-design.md`
- `docs/dev/policies/0014-roadmap-runbook-governance.md`
- `docs/dev/policies/0015-architecture-guardrails.md`
